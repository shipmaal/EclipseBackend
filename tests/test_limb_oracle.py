...
the cone radii L1'/L2', and the contact functions G_T / G_A.  This oracle
shares none of that.  From a sea-level observer (WGS-84 [WGS84], ITRS via ERFA
...
(this module's sites, docs sec. 9.6): <= 62 m; gate 75 m.
...
def _observer_itrs_km(lat_deg: float, lon_deg: float) -> np.ndarray:
    """Geodetic (h = 0) to ITRS [km] on the WGS-84 ellipsoid [WGS84]."""
    ...
    return np.array([n * np.cos(la) * np.cos(lo), n * np.cos(la) * np.sin(lo),
                     n * (1.0 - WGS84_E2) * np.sin(la)])


def oracle_h(dem: Dem, et: float, lat_deg: float, lon_deg: float, annular: bool = False,
             n_rays: int = 3600, half_span_km: float = 280.0, step_km: float = 0.25) -> float:
    ...
    obs = rc2t.T @ _observer_itrs_km(lat_deg, lon_deg)
...
# Sites: mid-path and within ~1.5 km of a limit, total and annular.
#   label, T0 (UTC, near the site's maximum), lat, lon [deg]
SITES = [
    ("2024 Indianapolis (mid-path)", "2024-04-08T19:08:00", 39.77, -86.15),
    ("2024 Effingham (~1 km inside the S limit)", "2024-04-08T19:03:38", 39.12, -88.55),
    ("2017 Vale OR (~1.5 km inside the S limit; Irwin et al. 2021)", "2017-08-21T17:25:50",
     43.953028, -117.219389),
    ("2023 annular, greatest eclipse", "2023-10-14T17:59:27", 11.4, -83.1),
]
...
def test_profile_contacts_match_the_3d_oracle(site, dem):
    ...
    label, t0, lat, lon = site
    ep.load_kernels()
    model = BesselianModel(t0_utc=t0)
    raw = local_raw(model, lat, lon, "profile")
    assert raw.central, label
    annular = raw.L2_x > 0
    for c, entering in ((raw.c2, True), (raw.c3, False)):
        g = _profile_g(model, lat, lon, np.array([c - 0.5 / 3600, c + 0.5 / 3600]))
        rate = abs(g[1] - g[0]) * 6378.137  # km/s
        dt = GATE_KM / rate
        before = oracle_h(dem, model.et0 + c * 3600 - dt, lat, lon, annular)
        after = oracle_h(dem, model.et0 + c * 3600 + dt, lat, lon, annular)
        if entering:
