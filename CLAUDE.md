...
| `circumstances.py` | Per-observer local circumstances from a `BesselianModel`. |
| `reference.py` | Parses `app/data.txt` (a reference central-line track). |
| `main.py` | FastAPI: `/health`, `/besselian`, `/central-line`, `/circumstances`; serves `frontend/` at `/ui`. |
...
5. **Frames & epochs.** Input epochs are **UTC**; SPICE `str2et` handles ΔT.
   Published Besselian elements are tabulated in **TDT/TT** (use `str2et(".. TDT")`
   to compare). `mu` follows the ephemeris-hour-angle convention and differs from
   some tables by ΔT of Earth rotation — see `ephemeris.py`.
...
- **[Meeus98]** J. Meeus, *Astronomical Algorithms*, 2nd ed., Willmann-Bell, 1998 — ch. 54.
...
- **[Espenak]** F. Espenak & J. Meeus, *Five Millennium Canon of Solar Eclipses:
  −1999 to +3000*, NASA/TP-2006-214141, 2006 — methodology and the `k1`/`k2` lunar radii.
